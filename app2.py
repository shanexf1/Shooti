...
from shooti.grader.grade import CKPT_DIR, load_grader
...
with st.sidebar:
    ...
    channels = st.selectbox(
        "Grader",
        ["both", "clip", "geo"],
        help="both = CLIP + geometry (most responsive to reframing). "
        "clip = image only (best raw grading). geo = geometry only (weak baseline).",
    )
